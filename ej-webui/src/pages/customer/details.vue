 ...
 * @Version: 1.0
 * @Autor: xuxu
 * @Date: 2021-09-09 11:39:05
 * @LastEditors: xuxu
 * @LastEditTime: 2021-09-10 15:17:25
-->
<template>
  <div class="details">
    <!-- 返回 -->
    ...
    <el-row :gutter="20" style="margin-top: 20px">
      <el-card class="box-card">
        ...
        <div class="text item" v-if="resData.email != ''">
          邮箱：{{ resData.email }}
        </div>
        ...
        <div class="text item">
          用户状态：
          <el-tag
            v-if="resData.status == '正常'"
            type="success"
            effect="dark"
            size="medium"
            >{{ resData.status }}</el-tag
          >
          <el-tag
            v-if="resData.status == '禁用'"
            type="warning"
            effect="dark"
            size="medium"
            >{{ resData.status }}</el-tag
    ...
    >
      <el-form :model="production" :rules="rules" ref="saveForm">
        ...
        <el-upload
          ...
          action=" http://106.14.236.173:8002/baseUser/alterUserface"
